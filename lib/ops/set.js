...
Set.prototype.exec = function(data){
	...
	var parent = this.p.parent().get(data);
	if(data._type_){
		data._type_.parent(parent);
	}
	else if(!parent._type_){
		this.p.parent().parent().get(data)._type_.dirty(true);
	}
	// TODO: The parent pointer needs to be set, and the dirty state
