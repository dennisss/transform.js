...
function Ins(p, data){
	...
	if(p instanceof Path)
		...
		this.p = new Path(p, 1); // TODO: Does this work with a len of 1 when interacting with other operations
...
Ins.prototype.exec = function(data){
	...
	if(cont instanceof Array){
		for(var i = this.data.length - 1; i >= 0; i--){
			...
			if(this.data[i]._type_)
				this.data[i].parent(cont);
		}
		...
		type = contP.parent().get(data)._type_;
	}
	else{
