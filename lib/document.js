...
function Document(data, v){
	...
	this.data = DataType.augment(data, this);
	this.v = v || 0;
...
/**
 * Starts a server that will synchronize many client computers
 */
Document.prototype.server = function(sock){
...
/**
 * Starts a client to listen for remote changes on the document
 *
 ...
 *
 * @param sock an EventEmitter based socket on which events will be received and sent
 */
	...
	for(var i = 0; i < op.length; i++){
		...
		if(!(o instanceof Operation)){ // Parse it
			if(o == null){
				...
				op[i] = Ins(o.p, DataType.augment(o.i, this) );
			}
			else if(o.m){
				op[i] =	Move(o.f, o.t, o.m);
			}
			else if(o.s){
				op[i] = Set(o.p, DataType.augment(o.s, this) );
			}
		}
	}
